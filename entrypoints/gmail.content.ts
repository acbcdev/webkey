...
import { GMAIL, SELECTORS, SHORTCUTS } from "@/lib/constants";
	...
	main() {
		...
		const isProjectorMode = () => {
			...
			const hashParams = new URLSearchParams(
				hash.split(GMAIL.HASH_SEPARATOR)[1] || "",
			);
		...
		hotkeys(SHORTCUTS.GMAIL.ACCOUNT_SWITCH, (event) => {
			...
			if (
				numberPressed >= GMAIL.MIN_ACCOUNT &&
				numberPressed <= GMAIL.MAX_ACCOUNT
			) {
		...
		function goToInbox() {
			...
			if (inboxButton) {
				const inboxLink = $<HTMLElement>(SELECTORS.GMAIL.INBOX_LINK, inboxButton);
				if (inboxLink) {
					inboxLink.click();
			...
			window.location.href = GMAIL.URLS.ACCOUNT_INBOX(parseInt(currentAccount, 10));
		}
	},
});
